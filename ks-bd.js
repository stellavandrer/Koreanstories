/* ks-bd.js — Améliorations des planches BD (histoireN-bd.html).
   ...
   - réutilise la pastille du haut en bouton « FR » (et déplace le favoris à gauche),
   - rend les bulles plus compactes sur mobile (n'écrasent plus les visages),
   - « déjà lu » en bas de la planche quand elle est lue,
   - SUIT LE COURS uniquement si la planche est ouverte DEPUIS le parcours
     (cours.html) ; depuis le hub Histoires, on n'avance PAS le parcours. */
(function () {
  ...
  function ensureBadge() {
    ...
    badge.className = 'bd-read-badge';
    badge.innerHTML = '<svg viewBox="0 0 24 24"><polyline points="20 6 9 17 4 12"/></svg> Déjà lu';
    host.appendChild(badge);
    return badge;
  }
  function showBadge() { ensureBadge().classList.add('show'); }

  var progKey = 'ks_bd_h' + N + '_prog';
  if (localStorage.getItem(progKey) === '100') showBadge();   /* déjà lue auparavant */

  /* Marque comme lue à la fin du scroll : "déjà lu" partout ; le cours
     n'avance QUE depuis le parcours. */
  var cur = parcoursMode ? CUR[N] : null;
  var marked = false;
  function markRead() {
    if (marked) return;
    try {
